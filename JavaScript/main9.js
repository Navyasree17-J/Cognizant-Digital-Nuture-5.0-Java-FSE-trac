...
// 1. Refactored Function leveraging Default Parameters
// If the category is left empty when calling this function, it defaults safely to "General Community"
...
const defaultParametersEvent = buildModernEventObject("Seniors Morning Coffee Walk"); // Uses capacities defaults!
...
// 3. Object Destructuring to extract values inline directly from objects cleanly
function displayModernEventDestructured(eventObject) {
    // Unpacks key parameters out into clean individual local scope variables immediately
...
// 4. Spread Operator Array Cloning & Manipulation
console.log("\nTesting Non-Mutating Array Clones via Spread Operator (...):");

const baseSourceArray = ["Art Expo", "Book Club", "Chess Match"];

// Create a brand-new distinct reference clone copy using the spread operator [...]
const clonedWorkingArray = [...baseSourceArray]; //

// Modify the cloned array list safely
