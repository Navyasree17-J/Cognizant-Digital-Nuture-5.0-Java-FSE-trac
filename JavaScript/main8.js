    ...
    searchTextAreaInput.addEventListener("input", (event) => {
        ...
        renderedCardsList.forEach((card) => {
            ...
            if (cardTitleText.includes(searchStringValue)) {
                card.parentElement.style.style.display = ""; 
...
// 1. Define a function that returns a Promise to simulate loading an external data feed
function fetchRemotePortalEvents() {
    return new Promise((resolve, reject) => { //
        console.log("📡 Asynchronous Stream: Connecting to remote server cloud data pipeline...");
        
        // Use a 2-second timer delay to simulate real-world web response latency
        setTimeout(() => { //
            const processingErrorFault = false; // Toggle to true to manually test failure states
            ...
            if (!processingErrorFault) {
                ...
                // Fulfill the promise safely, passing out our simulated raw data records
                resolve(simulatedCloudPayload); //
            } else {
                // Reject the promise if a simulated error happens
...
// 2. Build an async/await execution block to orchestrate the timeline cleanly
async function loadPortalDashboardAsynchronously() { //
    try {
        ...
        // Halt execution flow locally until the promise resolves successfully
        const freshCloudPayloadData = await fetchRemotePortalEvents(); //
        
        console.log("✨ Asynchronous Stream Success! Data packet received safely:");
        console.log(freshCloudPayloadData);

        // Inject the newly fetched data into our web view layout if the element exists
        ...
        // Gracefully catch and handle any promise rejections inside our try block
...
// 3. Kickoff the asynchronous execution flow engine automatically
