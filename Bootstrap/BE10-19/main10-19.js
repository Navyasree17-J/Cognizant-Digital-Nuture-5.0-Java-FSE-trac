...
// =========================================================================
// EXERCISE 9: INTERACTIVE BUTTON LOGIC & EVENT MONITORING
// =========================================================================
console.log(`--- Exercise 9: Semantic Button Event Engines Initialized ---`);

// 1. Select telemetry output monitor channel
const telemetryScreen = document.getElementById("liveTelemetryOutput");

// 2. Process Exercise 9.1: Multi-button query selections using .querySelectorAll()
const standardActionButtons = document.querySelectorAll(".log-btn");

// Loop through the node array collection and assign execution listeners to every item
standardActionButtons.forEach((buttonElement) => {
    buttonElement.addEventListener("click", () => {
        // Read out custom contextual identification tag stored on the HTML element
        const selectedButtonType = buttonElement.getAttribute("data-type");
        
        // Print to screen monitor workspace layout
        telemetryScreen.innerText = `> Click Detected on Contextual Component! Type Target: [${selectedButtonType}]`;
        console.log(`Telemetry Update: Standard button fired -> ${selectedButtonType}`);
    });
});

// 3. Process Exercise 9.2 (Part A): Segmented Button Group Active State Switcher
const filterGroupButtons = document.querySelectorAll(".filter-group-btn");

filterGroupButtons.forEach((groupButton) => {
    groupButton.addEventListener("click", () => {
        // Step 1: Strip the visual ".active" selection highlight from all buttons in the group
        filterGroupButtons.forEach(btn => btn.classList.remove("active"));
        
        // Step 2: Append the ".active" class layout onto the single clicked element item
        groupButton.classList.add("active");
        
        const currentViewFilterValue = groupButton.getAttribute("data-view");
        telemetryScreen.innerText = `> Button Group Modified! Active Segment Changed To View: [${currentViewFilterValue}]`;
        console.log(`Telemetry Update: Group Filter switched -> ${currentViewFilterValue}`);
    });
});

// 4. Process Exercise 9.2 (Part B): Advanced Checkbox Button Toggle Monitors
const interactiveCheckboxInputs = document.querySelectorAll(".toggle-node-input");

interactiveCheckboxInputs.forEach((checkboxInput) => {
    checkboxInput.addEventListener("change", () => {
        // Extract parameter context strings
        const targetLabelString = checkboxInput.getAttribute("data-label");
        
        // Read out true boolean tracking binary choice metric state (.checked returns true or false)
        const currentToggleState = checkboxInput.checked;
        
        telemetryScreen.innerText = `> Toggle Event Captured! Variable [${targetLabelString}] Set Status to: [${currentToggleState ? "ENABLED ✅" : "DISABLED ❌"}]`;
        console.log(`Telemetry Update: Toggle Checkbox "${targetLabelString}" -> ${currentToggleState}`);
    });
});

// =========================================================================
// CONSOLIDATED JAVASCRIPT MASTER APPLICATION ENGINE (EXERCISES 10 - 19)
// =========================================================================
console.log("--- Master Logic Engine Core Online ---");

// 1. Target and hook form components using unified DOM queries (Exercise 10 Navbar integration)
const searchForm = document.getElementById("navbarSearchForm");
const searchInputField = searchForm.querySelector("input");

searchForm.addEventListener("submit", (event) => {
    // Prevent default form data page reloads instantly inside memory
    event.preventDefault();
    
    const searchQueryString = searchInputField.value.trim();
    
    if (searchQueryString === "") {
        console.warn("⚠️ Search operation blocked: Empty data string query entered.");
        alert("Please input valid tracking parameters inside the filter box.");
        return;
    }
    
    console.log(`🔍 Master Router Processing Query Parameters: "${searchQueryString}"`);
    alert(`Query Registered!\nSearching database indices for: "${searchQueryString}"`);
    
    // Clear field after tracking processing complete
    searchForm.reset();
});

// 2. Initialize and monitor tabbed navigation layout changes programmatically
const tabControlButtons = document.querySelectorAll('#dashboardTabs button');

tabControlButtons.forEach((tabButton) => {
    tabButton.addEventListener('shown.bs.tab', (event) => {
        // event.target represents the newly activated tab element item
        const activeTabIdAttribute = event.target.id;
        console.log(`🗂️ Viewport context window re-aligned. Active Context Node: [${activeTabIdAttribute}]`);
    });
});

// 3. Dynamic initialization logic check for Exercise 19 compiled variables
(function verifySassEnvironmentParameters() {
    const systemDesignMetrics = {
        primaryColorTarget: "#0d6efd",
        customSassBorderRadius: "0.5rem",
        isSassRecompiledSuccess: true
    };
    
    console.log("🛠️ Sass System Configuration Map Parsed Successfully:", systemDesignMetrics);
})();